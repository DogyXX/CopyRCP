...
namespace RoverControlApp.Core
{
	public abstract class RoverControllerPresets
	{
		...
		{
			private bool lastGripperState = true;
			public bool CalculateMoveVector(out MqttClasses.ManipulatorControl manipulatorControl)
			{
				...
				float griperAxis = Input.GetAxis("manipulator_gripper_close", "manipulator_gripper_open");

				if (!Mathf.IsZeroApprox(griperAxis))
					lastGripperState = griperAxis < 0.0f;
